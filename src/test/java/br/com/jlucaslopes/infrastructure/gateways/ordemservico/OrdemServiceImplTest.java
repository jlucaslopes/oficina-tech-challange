...
class OrdemServiceImplTest {
    ...
    // associate saved service with the existing ordem entity and piece to avoid null in mapping
    savedServico.setOrdemServico(e);
    savedServico.setPeca(p);
    ...
    // associate saved service with the existing ordem entity to avoid null in mapping
    savedServico.setOrdemServico(e);
        when(servicoRepository.save(any())).thenReturn(savedServico);
    ...
    void avancarStatus_toEntregue_setsDataFim() {
        ...
        saved.setId(7L);
        saved.setDataFim(OffsetDateTime.now());
        when(ordemServicoRepository.saveAndFlush(any())).thenReturn(saved);
    ...
    void cancelarOrdem_success() {
        ...
        saved.setId(22L);
        saved.setDataFim(OffsetDateTime.now());
        saved.setStatus(Status.CANCELADA);
        when(ordemServicoRepository.saveAndFlush(any())).thenReturn(saved);
